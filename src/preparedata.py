...
class WikipediaPreparator():
    ...
    def savePage(self, dumpDirectoryPath, pageName, pageText, compress):
    ...
    def unpackDump(self, dumpIndex, dumpPath, outputDirectoryPath, cleanText, compress):
        ...
        try:
            ...
            pages = filter(self.filterPage, pages)

            if cleanText:
                pages = map(self.cleanPage, pages)

            for pageName, pageText in pages:
                self.savePage(dumpDirectoryPath, pageName, pageText, compress)
    ...
    def prepare(self, inputDirectoryPath, outputDirectoryPath, filterText=True, compress=True):
        ...
        for dumpIndex, dumpPath in enumerate(dumpPaths):
            pool.apply_async(self.unpackDump, args=(dumpIndex, dumpPath, outputDirectoryPath, filterText, compress))
        pool.close()
        pool.join()
